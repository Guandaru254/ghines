    ...
    return (
      <Fragment>
        ...
        <div style={{ textAlign: 'center', padding: '100px', minHeight: '400px' }}>
          ...
          <p>Check your Strapi connection and ensure content is published.</p>
        </div>
        <Scrollbar />
      </Fragment>
    );
...
// NO getStaticPaths - ONLY getStaticProps for static listing page
export async function getStaticProps() {
  if (!STRAPI_BASE_URL || !/^https?:\/\//.test(STRAPI_BASE_URL)) {
    console.error('[blog-fullwidth] Invalid STRAPI_BASE_URL');
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[blog-fullwidth] HTTP ${res.status}:`, errBody.substring(0, 200));
      return { props: { posts: [], pagination: {} }, revalidate: 60 };
    }
    ...
    console.log(`[blog-fullwidth] Successfully fetched ${formattedPosts.length} posts`);
    ...
    console.error('[blog-fullwidth] Critical error:', err.message);
    return { props: { posts: [], pagination: {} }, revalidate: 60 };
  }
}
